...
void LineFollower::lineFollowBackwards(){
	  ...
	  if(leftSensorValue >= ON_BLACK && rightSensorValue>= ON_BLACK)
	  {
	   if(canCountLine){
	      ...
	      // row or column
	      float headingInRadians = (robotChassis->myChassisPose.currentHeading)*(PI/180.0);
...
void LineFollower::lineFollowForwards(){
	  ...
	  if(leftSensorValue >= ON_BLACK && rightSensorValue>= ON_BLACK)
	  {
	   if(canCountLine){
	      ...
	      robotChassis->myChassisPose.colCount += round(sin(headingInRadians)); // have this in case we do double columns
